...
func seclangparserParserInit() {
	...
	staticData.SymbolicNames = []string{
		...
		"CONFIG_DIR_SEC_TMP_DIR", "CONFIG_DIRECTIVE_SEC_RULE", "DIRECTIVE_SECRULESCRIPT",
...
const (
	...
	SecLangParserCONFIG_DIRECTIVE_SEC_RULE             = 218
...
type IRules_directiveContext interface {
	...
	// Getter signatures
	CONFIG_DIRECTIVE_SEC_RULE() antlr.TerminalNode
...
func (s *Rules_directiveContext) CONFIG_DIRECTIVE_SEC_RULE() antlr.TerminalNode {
	return s.GetToken(SecLangParserCONFIG_DIRECTIVE_SEC_RULE, 0)
}
...
func (p *SecLangParser) Rules_directive() (localctx IRules_directiveContext) {
	...
	{
		p.SetState(282)
		p.Match(SecLangParserCONFIG_DIRECTIVE_SEC_RULE)
		if p.HasError() {
			// Recognition error - abort rule
			goto errorExit
		}
